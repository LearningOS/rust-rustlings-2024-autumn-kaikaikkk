...
impl<T> LinkedList<T> {
        ...
        T: PartialOrd + Clone,
    {
        ...
        let mut remaining_list = if current_a.is_none() { current_b } else { current_a };
        while remaining_list.is_some() {
            let value = unsafe { &(*remaining_list.unwrap().as_ptr()).val };
            merged_list.add(value.clone());
            remaining_list = unsafe { (*remaining_list.unwrap().as_ptr()).next };
        }
    ...
    T: Display,
{
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        ...
        while let Some(node) = current_node {
            ...
            current_node = unsafe { (*node.as_ptr()).next };
        }
        Ok(())
    }
}
