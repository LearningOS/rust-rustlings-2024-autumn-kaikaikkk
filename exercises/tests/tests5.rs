...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: It is the caller's responsibility to ensure that the address is valid
    // and points to a `u32` value that can be safely modified.
    unsafe {
        // Create a mutable reference to the memory location
        let value_ptr = address as *mut u32;
        
        // Dereference the pointer and modify the value
        ptr::write_volatile(value_ptr, 0xAABBCCDD);
    }
}
